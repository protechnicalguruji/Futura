            ...
            >
              ...
              <div className={`grid lg:grid-cols-2 gap-16 items-center ${i % 2 !== 0 ? 'lg:grid-flow-dense' : ''}`}>
                <div className={`relative group ${i % 2 !== 0 ? 'lg:col-start-2' : ''}`}>
                    <img src={p.image} alt={`Luxury project: ${p.name}`} className="w-full h-[600px] object-cover rounded-[28px] shadow-2xl" />
                ...
                <div className={`${i % 2 !== 0 ? 'lg:col-start-1' : ''}`}>
                    <h3 className="font-serif text-[48px] tracking-tighter mb-4" id={`project-${i}`}>{p.name}</h3>
                    <p className="text-[18px] text-text-secondary mb-8 leading-relaxed">{p.description}</p>
                    
                    <div className="grid grid-cols-2 gap-6 mb-10">
                        <div>
                            <div className="text-text-secondary text-[14px] uppercase mb-1">Starting Price</div>
                            <div className="font-medium text-[20px]">{p.price}</div>
                        </div>
                        <div>
                            <div className="text-text-secondary text-[14px] uppercase mb-1">Possession</div>
                            <div className="font-medium text-[20px]">{p.possession}</div>
                        </div>
                    </div>
                    <div className="flex gap-4">
                        <button className="bg-text-primary text-white px-10 py-4 rounded-full text-[15px] hover:bg-accent transition-all">Explore Project</button>
                        <button className="border border-border px-10 py-4 rounded-full text-[15px] hover:bg-bg-body transition-all">Download Brochure</button>
                    </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {p.amenities.map(a => {
                    ...
                    return (
                        <div key={a} className="bg-white p-8 rounded-[28px] border border-border flex items-center gap-4 hover:border-accent transition-all">
